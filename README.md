Ниже готовый текст для видео (под запись экрана) по первому боту (кулинарный помощник). Читайте его вслух, делая паузы. В квадратных скобках – действия на экране. Текст рассчитан на 5–7 минут.
...
[На экране – заставка или просто чат Telegram с ботом]

Голос:
Здравствуйте, уважаемые инвесторы. Меня зовут Дамир, и я представляю проект – Telegram-бот «Что съесть?».

Этот бот решает очень простую, но ежедневную проблему миллионов людей: что приготовить из тех продуктов, которые уже есть в холодильнике?

Целевая аудитория – все, кто готовит дома: от студентов до семей с детьми. Проблема в том, что люди тратят в среднем 10–15 минут в день на поиск рецептов или просто ломают голову над ужином. А ещё часто выбрасывают продукты, потому что не знают, что из них сделать.

Наш бот с помощью искусственного интеллекта GigaChat за пару секунд придумывает блюдо и даёт короткий рецепт. Пользователь просто перечисляет продукты – и получает готовое решение.

---

Демонстрация работы (1:00–3:30)

[Открываю Telegram, показываю чат с ботом @cook_assistant_bot]

Голос:
Сейчас покажу, как это работает на практике.

Я открываю Telegram и нахожу нашего бота. Отправляю команду /start.

[Пишу /start, показываю ответ]

Бот отвечает, что для доступа нужно ввести пароль – это наша система авторизации, чтобы ботом пользовались только те, кому мы разрешили.

Ввожу пароль: /login demo123.

[Пишу /login demo123, бот подтверждает]

Авторизация прошла успешно. Теперь отправляю список продуктов: курица, картошка, лук, сметана, соль.

[Пишу список, бот отвечает «Готовлю рецепт…»]

Бот начинает обработку – отправляет запрос в ИИ GigaChat. Примерно через 2–3 секунды получаем ответ.

[Появляется ответ бота: название блюда и рецепт]

Как видите, бот предложил блюдо «Курица по-домашнему в сметанном соусе» и подробный рецепт из трёх шагов. Никакой воды – только конкретные действия.

Если бы продуктов не хватало, бот предложил бы добавить, например, морковь или чеснок, и всё равно дал бы рецепт. Это очень удобно, когда холодильник почти пуст.

---

Дополнительная функция – история запросов (3:30–4:30)

[Показываю команду /history]

Голос:
У бота есть ещё одна полезная функция – история. Команда /history показывает последние пять запросов и рецептов, которые вы делали.

[Ввожу /history, показываю вывод]

Это помогает не забыть удачные блюда и вернуться к ним через день-два. Все данные сохраняются в защищённой базе данных SQLite.

---

Ценность и перспективы (4:30–6:30)

[Статичный кадр или логотип]

Голос:
Какую измеримую ценность даёт наш продукт?

Во-первых, экономия времени: пользователь тратит не 15 минут на придумывание ужина, а 15 секунд. Допустим, человек готовит 5 раз в неделю. Это экономия более часа в неделю, 4–5 часов в месяц.

Во-вторых, экономия денег: бот помогает использовать остатки продуктов, сокращая пищевые отходы. По статистике, среднестатистическая семья выбрасывает до 20% купленных продуктов. Наш бот снижает этот процент.

Планы развития:

· Добавить распознавание еды по фото – сфотографировал содержимое холодильника, бот сам распознал продукты и предложил рецепт.
· Интеграция с доставкой продуктов – если не хватает ингредиента, бот предложит заказать его в ближайшем магазине.
· Подписка Pro – для тех, кто хочет сохранять рецепты в личный кабинет, получать персональные подборки и отключать рекламу.

Мы планируем выйти на рынок в модели freemium: базовый функционал бесплатный, расширенные возможности – за небольшую плату (например, 199 рублей в месяц).

Также есть потенциал продажи белой метки сервисам доставки готовой еды и кулинарным блогам.

---

Заключение (6:30–7:00)

[Снова экран чата с ботом или финальный слайд]

Голос:
Наш продукт уже полностью рабочий, протестирован и готов к масштабированию. Мы используем российский ИИ GigaChat, что даёт нам технологическую независимость и низкую себестоимость.

Я приглашаю вас инвестировать в развитие и вывод этого бота на рынок. Буду рад ответить на вопросы.
...
Вы можете прочитать этот текст ровно в таком же темпе, как я написал, с естественными паузами. Общее время получается около 7 минут. Если нужно уложиться строго в 10 минут – добавьте на демонстрацию ещё один пример продукта (например, «яйца, помидоры, сыр»). Удачи с записью!
...
https://drive.google.com/file/d/1TE2mS6_D8qDxsUDB3XyXgBVZUzZLVlmM/view?usp=sharing




https://drive.google.com/file/d/1lDJC-U9SGTtEOcT72AP1q-tr634wWY-3/view?usp=sharing
